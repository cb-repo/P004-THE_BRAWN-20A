...
uint16_t SBUS_CalculateCS(uint8_t *);
bool SBUS_VerifyCS(uint8_t *);
...
void SBUS_Init(UART_t u)
{
	uart = u;
//	UART_Mode_t mode = UART_Mode_Default;
//	UART_Init(&uart, SBUS_BAUD, mode);
}
 ...
 */
uint8_t SBUS_Update(uint16_t * data)
{
...
uint16_t SBUS_CalculateCS(uint8_t * data)
{
...
bool SBUS_VerifyCS(uint8_t * data)
{
